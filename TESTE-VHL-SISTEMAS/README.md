...
*   **Escolha do Zabbix para Observabilidade:** A escolha do Zabbix como solução de monitoramento deve-se ao fato de eu já utilizar e trabalhar ativamente com esta ferramenta no meu emprego atual. Essa familiaridade e ambientação profissional facilitaram a implantação rápida do Zabbix Appliance (consolidando o Server, banco e web em um único pod) e a configuração ágil do cenário web para testar a aplicação de ponta a ponta.
